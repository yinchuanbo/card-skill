...
function initDynamicFavicon() {
  ...
  function createAnimatedFavicon() {
    ...
    const faviconUrl = originalFavicon.getAttribute("href");
    ...
    img.onload = function () {
      ...
      // 清除已有的动画循环
      if (faviconPulse) clearInterval(faviconPulse);

      // 设置动画帧
      faviconPulse = setInterval(() => {
        // 清空画布
  ...
  // 页面加载状态变化时动态更新favicon
  window.addEventListener("load", () => {
    if (faviconPulse) {
      clearInterval(faviconPulse);

      // 恢复原始favicon
      const originalFavicon = document.querySelector('link[rel="icon"]');
      if (originalFavicon) {
        originalFavicon.href = "/images/icons/favicon.svg";
      }
    }
  });
...
function initElegantScrolling() {
  // 为内容区域添加平滑滚动
  document
    .querySelectorAll(".card-content, #modalContent")
    .forEach((element) => {
      if (element) {
        // 添加滚动监听
...
function addScrollDecoration(element) {
  ...
  if (element.scrollHeight > element.clientHeight) {
    ...
    decoration.style.cssText = `
            ...
            background-repeat: no-repeat;
            opacity: ${
              scrollPercentage > 5 && scrollPercentage < 95 ? "0.4" : "0"
            };
...
function addGlobalScrollEffect() {
  ...
  // 应用视差滚动效果到背景元素
  if (bodyBg) {
    bodyBg.style.transform = `translateY(${
      scrollPosition * 0.03
    }px) rotate(10deg)`;
  }
  ...
  decorElements.forEach((elem) => {
    const randomFactor = parseFloat(
      elem.dataset.scrollFactor || Math.random() * 0.05
    );
    elem.style.transform = `translateY(${
      scrollPosition * randomFactor
    }px) rotate(${elem.dataset.rotation || "0deg"})`;
  });
}
